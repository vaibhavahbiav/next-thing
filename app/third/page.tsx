...
const ThirdPage = () => {
  ...
  return (
    <div className='flex flex-col space-y-18 pt-10 mx-52 transition-all'>
      ...
      <div className='border-l-2 border-r-2 border-teal-800 py-10 '>
        <form className='flex items-center justify-center space-x-9' autoComplete='off' onSubmit={handleSubmit}>
          <div className='flex-col flex space-y-7 ml-9 w-1/3'>
            ...
            <div className='relative tracking-widest bg-cyan-800 w-full after:absolute after:top-0 after:right-0 after:content-[""] after:size-8 after:z-10 after:bg-teal-600 after:border-b-4 after:border-l-4 after:border-teal-100 shadow-lg shadow-teal-300'>
              <textarea className='bg-teal-200 border-l-8 border-teal-200 focus:border-l-8 resize-none focus:border-cyan-950 focus:outline-0 w-full text-teal-950 px-5 pt-10 pb-3 h-[200px] transition-all' name='message' id='message' value={message} onChange={handleChange} />
          ...
          <button type="submit" onClick={handleClick} className={`pt-9 tracking-widest font-thin text-lg uppercase cursor-pointer transition-all relative after:absolute after:top-0 after:right-0 :content-[""] after:size-6 after:z-10 after:border-b-4 after:border-l-4 after:border-teal-100 ${clicked ? 'after:block' : 'after:hidden'} ${messageError ? 'bg-rose-800 text-red-100 shadow-lg shadow-red-400 after:bg-red-800 hover:bg-rose-800' : 'bg-teal-200 text-cyan-200 hover:bg-cyan-800 after:bg-teal-600'}`}><span className={`px-3 py-1 ${messageError ? 'bg-rose-800' : 'bg-cyan-800'}`}>{buttonText}</span></button>
        </form>
      </div>
    </div>
  )
